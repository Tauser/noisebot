...
- micro-drift, saccades e tilt sustentado; ✓ **implementado e estabilizado
  em bancada** em 2026-06-22. `gaze_service.c` calcula
  ...
  **Problema encontrado e corrigido**: ao ligar o gaze, o enlace oscilava
  `READY` ↔ `DEGRADED` a cada ~7-15s. A captura paralela da COM12 mostrou
  que não era congestionamento do link: o task watchdog abortava o head
  porque `nb_head_display` mantinha o CPU 1 ocupado e `IDLE1` não rodava.
  Cada cena de gaze reiniciava indevidamente a interpolação facial de
  220 ms; como chegavam cenas continuamente, o renderer nunca atingia o
  estado ocioso e mantinha `pushSprite()` contínuo. A correção separa
  mudança de expressão de atualização de gaze/overlay/brilho: somente
  expressão reinicia a interpolação; gaze força um único redesenho. A
  task também passou de `vTaskDelayUntil()` para `vTaskDelay()`, garantindo
  yield real mesmo se o push do frame ultrapassar o período nominal.
  O deadband de 10 milli-units permanece como redução útil de tráfego,
  mas não era a correção da causa raiz. `gaze_service_init_standalone()`
  foi reativada no perfil Waveshare. Validação física: soak de 90 s com
  gaze + blink ativos, link sempre `READY`, 669 TX / 660 RX na última
  telemetria, zero retry, timeout, erro SPI, watchdog ou reboot;
...
fechado** — gaze dinâmico + blink passaram por soak técnico de 90 s sem
reset ou degradação, mas ainda faltam motifs compostos, timelines com
prioridade/interrupção e a validação visual completa segundo
