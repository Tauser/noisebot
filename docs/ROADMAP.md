...
| F4 câmera | DM4 | `EM ANDAMENTO` | DM4.1–DM4.4 concluídas: contrato semântico, round trip command/event no head, cliente no main e probe opt-in (`CONFIG_NB_DM4_CAMERA_PROBE`) provando o roteamento ponta a ponta, tudo sem hardware; builds main/head verdes com `-Werror`. Ver `docs/DM4_CAMERA_MIGRATION.md` (inclui gate de bancada DM4.4). Falta HAL DVP físico, preview local e wirear a um consumidor de produto |
