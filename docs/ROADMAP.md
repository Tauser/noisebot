...
| Touch sensibilidade e confiabilidade (2.2A) | `FEITO` — default em 8 (1,6% threshold): rejeita proximidade de fio/antena (~1% EMA) sem perder contato real (~1,5% EMA em 60ms); legados 10, 25 e 5 migrados; startle threshold 0.3→0.15 | `components/services/touch_service/`, `components/infra/nb_config_keys.h` |
