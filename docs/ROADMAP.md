...
**Status atual:** iniciada. O bridge mantém handshake v1 vazio e anuncia `HELLO` v2 em runtime; firmware atualizado responde com capabilities v2 quando recebe esse `HELLO`, preservando compatibilidade com bridge v1. A primeira telemetria v2 de sessão já registra `WAKE_DETECTED`, `LISTEN_START`, `LISTEN_STOP`, `SESSION_DONE` e `SESSION_ERROR` no bridge, derivada do fluxo v1 existente.
