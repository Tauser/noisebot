...
- [x] FPS de render mantido ≥ 30fps com câmera ativa por teste curto de 120s
...
- [ ] Fase N4.3 iniciada com provider explicito de frame SAY para speaker:
  ...
  preenche um chunk fornecido pelo `audio_service`, que continua aplicando
  volume, escrevendo no HAL e anotando o resultado no Audio IO v2. O objetivo e
  mover a selecao/fornecimento do frame para Playback v2 sem ainda transferir o
...
- [x] Fase N4.3 validada em hardware apos ajuste `5dfce7a`: com owner armado,
  ...
  para `DISABLED`.
- [x] Fase N4.4 validada em hardware: Playback v2 passou a preparar o frame SAY do speaker
  ...
  `idf.py build` sem warnings. Validacao fisica apos flash: gate armado com
  `speaker_owner_ready=true`, turno curto real gerou 380 frames preparados,
  97280 samples preparados, `speaker_last_samples=256`, `speaker_last_volume=80`,
  380/380 chunks SAY recebidos/tocados, fila final zero, zero drops,
...
- [x] Sessão ativa de câmera exibe ícone visual discreto, compondo com o overlay de microfone sem sobreposição.
