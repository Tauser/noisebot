...
/* Barge-in durante TTS do bridge. O VAD conversacional fica mutado enquanto o
 * speaker toca para não reagir ao próprio áudio; este gate observa apenas fala
 * forte e sustentada acima do vazamento acústico do TTS. */
...
#define BARGE_IN_MIN_RMS_ABS    48000.0f
#define BARGE_IN_LEAK_MULT          1.50f
...
static struct {
    ...
    uint32_t             bridge_barge_elapsed_ms;
    uint32_t             bridge_barge_cooldown_ms;
    uint8_t              bridge_barge_score;
    float                bridge_barge_leak_ema;
    ...
    if (s.bridge_say_playing && local_output_active && !s.listen_session_active) {
        if (s.bridge_barge_cooldown_ms > 0U) {
            ...
            if (s.bridge_barge_score >= BARGE_IN_SCORE_START) {
                ESP_LOGI(TAG,
                         "barge-in durante SAY rms=%ld thr=%ld leak=%ld",
                ...
                if (audio_service_begin_listen_session(NB_LISTEN_SOURCE_BARGE_IN) != ESP_OK) {
                    ESP_LOGW(TAG, "barge-in detectado, mas nao abriu sessao");
                }
            }
        }
