 ...
 *
 * Backend esp_video/V4L2 inspirado no StackChan. A camera DVP compartilha
...
{
    return (mode == NB_CAMERA_MODE_BETTER_QVGA) ? 320U : 160U;
}

size_t camera_hal_mode_height(nb_camera_mode_t mode)
{
    return (mode == NB_CAMERA_MODE_BETTER_QVGA) ? 240U : 120U;
}
...
esp_err_t camera_hal_init(void)
{
    ...
    if (pixfmt != V4L2_PIX_FMT_YUV422P && pixfmt != V4L2_PIX_FMT_JPEG) {
    ...
    if (ioctl(s_video_fd, VIDIOC_G_FMT, &current_fmt) != 0) {
        ESP_LOGW(TAG, "VIDIOC_G_FMT falhou errno=%d (%s)", errno, strerror(errno));
    } else {
        ESP_LOGI(TAG, "formato atual V4L2: %lux%lu fmt=0x%08lx",
                 (unsigned long)current_fmt.fmt.pix.width,
                 (unsigned long)current_fmt.fmt.pix.height,
                 (unsigned long)current_fmt.fmt.pix.pixelformat);
    }
...
esp_err_t camera_hal_capture(void)
{
    ...
    for (uint32_t attempt = 0; attempt < NB_CAMERA_CAPTURE_ATTEMPTS; ++attempt) {
        ...
        if (attempt < NB_CAMERA_WARMUP_FRAMES) {
            (void)ioctl(s_video_fd, VIDIOC_QBUF, &buf);
            vTaskDelay(pdMS_TO_TICKS(50));
            continue;
        }
