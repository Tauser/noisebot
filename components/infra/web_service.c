...
static esp_err_t send_audio_processor_status(httpd_req_t *req, esp_err_t probe_err)
{
    ...
    snprintf(buf, sizeof(buf),
             ...
             "\"sample_rate_hz\":%d,"
             "\"opus_frame_duration_ms\":%d,"
             "\"opus_encoder_frame_bytes\":%d,"
             "\"opus_encoder_out_bytes\":%d,"
             "\"opus_encoded_bytes\":%d,"
             "\"opus_decoded_bytes\":%d,"
             ...
             st.fetch_channels,
             st.sample_rate_hz,
             st.opus_frame_duration_ms,
             st.opus_encoder_frame_bytes,
             st.opus_encoder_out_bytes,
             st.opus_encoded_bytes,
             st.opus_decoded_bytes,
...
static esp_err_t handle_api_audio_processor_opus_probe(httpd_req_t *req)
{
    if (audio_service_is_busy()) {
        httpd_resp_set_type(req, "application/json");
        httpd_resp_set_status(req, "409 Conflict");
...
static const httpd_uri_t k_uris[] = {
    ...
    { .uri = "/api/audio/processor/opus/probe", .method = HTTP_POST, .handler = handle_api_audio_processor_opus_probe },
