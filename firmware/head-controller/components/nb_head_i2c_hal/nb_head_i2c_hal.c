...
static const char *TAG = "nb_head_i2c_hal";
...
{
    if (s_initialized) {
        ESP_LOGW(TAG, "barramento ja inicializado");
        return ESP_ERR_INVALID_STATE;
    }

    i2c_master_bus_config_t bus_cfg = {
        .i2c_port = NB_HEAD_I2C_PORT,
        .sda_io_num = NB_HEAD_PIN_I2C_SDA,
        .scl_io_num = NB_HEAD_PIN_I2C_SCL,
    ...
    ESP_LOGI(TAG, "bus I2C iniciado - SDA=%d SCL=%d freq=%dkHz",
             NB_HEAD_PIN_I2C_SDA, NB_HEAD_PIN_I2C_SCL,
             NB_HEAD_I2C_FREQ_HZ / 1000);
    return ESP_OK;
}
...
esp_err_t nb_head_i2c_hal_scan(uint8_t *found, size_t max_found, size_t *count)
{
    ...
    if (found_count == 0) {
        ...
        ESP_LOGI(TAG, "scan concluido: %zu dispositivo(s)", found_count);
    }
