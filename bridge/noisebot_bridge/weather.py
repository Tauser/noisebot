...
def format_weather_reply(weather: WeatherNow) -> str:
    ...
    location = weather.location or "sua localização configurada"
    label = _WEATHER_LABELS.get(weather.weather_code or -1)
    temp = int(round(weather.temperature_c))
    if label:
        return f"Agora em {location} está {temp} graus, com {label}."
    return f"Agora em {location} está {temp} graus."
...
def fetch_weather_now(timeout_s: float = 3.0) -> WeatherNow:
    ...
    location = os.environ.get("NOISEBOT_WEATHER_LOCATION", "Brasília")
    params = urlencode(
        {
            "latitude": lat,
            "longitude": lon,
